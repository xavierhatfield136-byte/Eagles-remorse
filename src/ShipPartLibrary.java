...
final class ShipPartLibrary {
    ...
    static boolean hasParts(ShipRole role, Faction faction) {
        return getSet(role, faction).hasParts();
    }
    ...
    static PartSet getSet(ShipRole role, Faction faction) {
        ...
        for (int i = 1; i <= 8; i++) {
            ...
            PartSprite sprite = loadVariant(roleKey, factionKey, idx);
            if (sprite == null) break;
            parts.add(sprite);
        }

        PartSet set = new PartSet(parts);
        CACHE.put(cacheKey, set);
        return set;
    }

    private static PartSprite loadVariant(String roleKey, String factionKey, String idx) {
    ...
    private static PartSprite loadImage(String key) {
        ...
        PartSprite sprite = trimSprite(img);
        if (sprite != null) IMAGE_CACHE.put(key, sprite);
        return sprite;
    }

    private static PartSprite trimSprite(BufferedImage src) {
        ...
        return new PartSprite(trimmed, offsetXNorm, offsetYNorm, widthNorm, heightNorm);
    }
    ...
    static final class PartSprite {
        ...
        PartSprite(BufferedImage image, double offsetXNorm, double offsetYNorm, double widthNorm, double heightNorm) {
            this.image = image;
